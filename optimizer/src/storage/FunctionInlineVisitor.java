...
public class FunctionInlineVisitor extends ReplacingASTVisitor implements OptimizerVisitor {
    ...
    public FunctionInlineVisitor(OptimizerContext context, OptimizationDecider decider) {
        super(context.parserContext(), true, true);
        this.branchRegistry = context.branchRegistry();
    ...
    private Node singleStatementFunction(FunctionCallNode functionCall, StatementListNode statements) {
        ...
        if (needWrapping(expression)) {
            expressionToInline = new ParenthesisNode(expressionToInline.getSourcePosition(), expressionToInline);
        }
