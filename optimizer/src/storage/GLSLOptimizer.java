...
public class GLSLOptimizer {
    ...
    public GLSLOptimizer(OutputWriter outputWriter, OutputProfile outputProfile) {
        ...
        this.outputConfig = null; // TODO: initialize
    ...
    public void process(List<GLSLSource> shaderSources) {
        List<OptimizerContext> contexts = new ArrayList<>();
        ...
        for (OptimizerContext context : contexts) {
            output("--------------------------------------------------\n");
    ...
    */
    /*
    private OptimizerContext execute(GLSLSource glslSource) {
        String shaderFilename = glslSource.getFilename();
        String shaderSource = glslSource.getContent();

        final OptimizerContext context = new OptimizerContext(shaderFilename);
    ...
    */
    ...
    private void writeShader(OptimizerContext context, boolean multipleShaders) {
        ...
        if (!option(NoIdentifierRenaming)) {
            ...
            while (true) {
                final boolean loop = false; //!passThrough && identifierShortener.permutateIdentifiers();
        ...
        */
        String content = null; //generator.generate(context, outputShader);
    ...
    private FileGenerator createGenerator(OutputProfile outputProfile, String shaderFilename, boolean multipleShaders, boolean bitPack) {
        ...
        throw new IllegalArgumentException("Unknown OutputProfile!");
            ...
            */
    private Node optimize(OptimizerContext optimizerContext, Node shaderNode) {
        ...
        final OptimizerPipeline pipeline = null; /*new BranchingOptimizerPipeline(pruner, outputConfig, optimizerTypes);*/
        return pipeline.optimize(optimizerContext, shaderNode, !option(SilentOutput));
    }

    private void shortenIdentifiers(OptimizerContext context) {
