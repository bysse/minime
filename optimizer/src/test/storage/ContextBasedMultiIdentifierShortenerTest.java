...
import com.tazadum.glsl.optimizer.Optimizer;
import com.tazadum.glsl.optimizer.constants.ConstantPropagation;
import com.tazadum.glsl.output.IdentifierOutput;
import com.tazadum.glsl.output.OutputConfig;
import com.tazadum.glsl.optimizer.OutputSizeDecider;
import com.tazadum.glsl.parser.ParserContext;
import com.tazadum.glsl.parser.TestUtil;
...
public class ContextBasedMultiIdentifierShortenerTest {
    ...
    public void test_basic_1() {
        ...
        Node node1 = compile(TestUtil.parserContext(), "uniform int x;void main(){float y=tan(0.0)*tan(0.0);}");
        Node node2 = compile(TestUtil.parserContext(), "void main(){gl_FragColor=vec3(1.0);}");
    ...
    public void test_basic_2() {
        config.setIdentifiers(IdentifierOutput.None);
        Node node1 = compile(TestUtil.parserContext(), "uniform vec2 X; void main(){ for (int Y=0;Y<10;Y++) { X.x += 1; }}");
        Node node2 = compile(TestUtil.parserContext(), "uniform vec2 X; void main() { gl_FragColor = vec3(X.xy, 0); }");
    ...
    public void test_basic_3() {
        config.setIdentifiers(IdentifierOutput.None);
        Node node1 = compile(TestUtil.parserContext(), SHADER_1);
    ...
    public void test_basic_4() {
        ...
        Node node1 = compile(TestUtil.parserContext(),SHADER_1);
        Node node2 = compile(TestUtil.parserContext(),SHADER_2);
    ...
    public void test_basic_5() {
        config.setIdentifiers(IdentifierOutput.None);
        Node node1 = compile(TestUtil.parserContext(), "uniform vec2 a; float x=a.x; float y=a.y; vec2 f(float b){return b*a*x/y;}void main(){ gl_FragColor.xy = f(2);}");
    ...
    public void test_for_loops() {
        config.setIdentifiers(IdentifierOutput.None);
        Node node1 = compile(TestUtil.parserContext(),
                "" +
                        "void main(){" +
                        " vec4 color=vec4(1);" +
                        " for(int i=0;i<5;i++) {" +
